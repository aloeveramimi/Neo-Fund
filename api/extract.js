...
10. note: Fill this with the mapped name of the members involved + specific item/reason (e.g., "Megan bún riêu").
    ...
    - NO GUESSWORK & NO AUTOMATIC NAMES: If there are NO member names on the bill (only random transaction codes, machine-generated numbers, internet banking automatic codes, or POS terminal IDs), do NOT guess, and NEVER automatically insert the uploader's name ("${userSelected}") or the extracted "from" name into the note. Just output the isolated reason/item (e.g., "BMTT", "Atiso Langfarm", McDonald) so the user can manually fill the name later.
